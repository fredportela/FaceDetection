...
///
    ...
    CascadeClassifier eyeDetector = new CascadeClassifier(FaceDetection.class.getResource("haarcascade_eye_tree_eyeglasses.xml").getPath().substring(1));
                ...
                while (runnable) {
                    if (webSource.grab()) {
                        try {
                            ...
                            //BUSCAR NO DB HASH IGUAL A IMAGEM CAPTURADA
                            System.out.println(frame.hashCode());
                
                            for (Rect rect : faceDetections.toArray()) {
                                //System.out.println(rect.hashCode());
                                //obtem os dados de onde estao as faces (altura, largura, posicao x e y)
                                faceFileSave(rect, frame);
                            }
                            ...
                            if (g.drawImage(buff, 0, 0, getWidth(), getHeight()-150 , 0, 0, buff.getWidth(), buff.getHeight(), null)) {
                                //g.drawString("TESTE", rect.x, rect.y);
                                if (runnable == false) {
    ...
    //GRAVA IMAGEM DETECTADA
    private void faceFileSave(Rect rect,Mat frame){
        try {
            Integer hash = frame.hashCode();
            
            //CRIA QUADRADO VERDE NO ROSTO
            Core.rectangle(frame, new Point(rect.x, rect.y), new Point(rect.x + rect.width, rect.y + rect.height), new Scalar(0, 255,0));    
            //CRIA O TEXTO ABAIXO DO QUADRADO
            Core.putText(frame, hash.toString(), new Point(rect.x+(rect.width/3), rect.y + rect.height + 20),  Core.FONT_HERSHEY_SIMPLEX, 0.5 ,new  Scalar(255, 255, 255));
                                
            String nome_arquivo = frame.hashCode()+".jpg";
            File outputfile = new File(nome_arquivo);
            ...
            if(!outputfile.exists()){
                ...
                BufferedImage imagemOriginal = Util.converterParaImage(frame);
                //corta os rostos da imagem desfocada, 
                ServiceCorteImagem serviceCrop = new ServiceCorteImagem();
                BufferedImage imagemSemEfeitos = serviceCrop.CortarImagemOriginal(prop, imagemOriginal);

                if(imagemSemEfeitos != null){
    ...
    public final void drawCenteredString(Graphics g, String text, Rectangle rect) {
        // Get the FontMetrics
